...
function addCustomLayers() {
  ...
  if (!map.getSource("batas_wilayah")) {
    ...
    map.addLayer({
      ...
      layout: {
        ...
        "text-size": 11,
        visibility: document.getElementById("layer-batas-kelurahan").checked
          ? "visible"
          : "none",
      },
      paint: {
    ...
    map.addLayer({
      ...
      layout: {
        ...
        visibility: document.getElementById("layer-batas-kecamatan").checked
          ? "visible"
          : "none",
      },
      paint: {
  ...
  if (!map.getSource("bidang_tanah")) {
    ...
    let bidangVis = document.getElementById("layer-bidang").checked
      ? "visible"
      : "none";
      ...
      .then((data) => {
        ...
        data.features.forEach((f) => {
          ...
          const enforceCounterClockwise = (ring) => {
            ...
            if (MathArea > 0) {
              // If > 0, it's clockwise. Needs to be reversed!
              ring.reverse();
            }
            return MathArea;
          };
...
document.querySelectorAll('input[name="sys-mode"]').forEach((el) => {
  el.addEventListener("change", (e) => {
    ...
    if (labelEl) {
      labelEl.textContent =
        currentMode === "bpn" ? "Bidang Tanah" : "Bidang Tanah";
    }
    ...
    if (
      currentThematic !== "none" ||
      document.getElementById("layer-bidang").checked
    ) {
...
document.getElementById("layer-bidang").addEventListener("change", (e) => {
  ...
  if (map.getLayer("bidang_tanah_fill"))
    map.setLayoutProperty("bidang_tanah_fill", "visibility", vis);
  if (map.getLayer("bidang_tanah_line"))
    map.setLayoutProperty("bidang_tanah_line", "visibility", vis);
});

document
  .getElementById("layer-batas-kelurahan")
  .addEventListener("change", (e) => {
    const vis = e.target.checked ? "visible" : "none";
    if (map.getLayer("batas_wilayah_line"))
      map.setLayoutProperty("batas_wilayah_line", "visibility", vis);
    if (map.getLayer("batas_wilayah_label"))
      map.setLayoutProperty("batas_wilayah_label", "visibility", vis);
  });

document
  .getElementById("layer-batas-kecamatan")
  .addEventListener("change", (e) => {
    const vis = e.target.checked ? "visible" : "none";
    if (map.getLayer("batas_kecamatan_line"))
      map.setLayoutProperty("batas_kecamatan_line", "visibility", vis);
    if (map.getLayer("batas_kecamatan_label"))
      map.setLayoutProperty("batas_kecamatan_label", "visibility", vis);
  });
...
document.querySelectorAll('input[name="thematic-layer"]').forEach((el) => {
  el.addEventListener("change", (e) => {
    ...
    if (map.getLayer("rdtr_fill"))
      map.setLayoutProperty("rdtr_fill", "visibility", "none");
    if (map.getLayer("znt_fill"))
      map.setLayoutProperty("znt_fill", "visibility", "none");

    // Show selected layer
    if (currentThematic !== "none") {
      if (map.getLayer(`${currentThematic}_fill`)) {
        map.setLayoutProperty(
          `${currentThematic}_fill`,
          "visibility",
          "visible",
        );
      }
...
map.on("click", (e) => {
  ...
  for (let key in props) {
    if (key === "layer" || key === "source" || key === "_NILBULAT_NUM")
      continue;
  ...
  if (layerTitle === "BIDANG_TANAH") {
    layerTitle = currentMode === "bpn" ? "BIDANG TANAH" : "BIDANG TANAH";
  }

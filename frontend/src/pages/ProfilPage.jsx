...
export default function ProfilPage() {
  ...
  const handleSaveProfile = () => {
    alert("Perubahan profil berhasil disimpan! (Logic akan diimplementasikan nanti)");
  };
  ...
  const handleFotoUpload = (e) => {
    ...
    if (file) {
      alert(`Foto "${file.name}" berhasil diupload (Logic akan diimplementasikan nanti)`);
    }
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-text-tertiary text-sm mt-1">Kelola informasi akun dan pengaturan keamanan</p>
      </div>

      {/* Profile Card */}
      <div className="bg-surface rounded-xl border border-border overflow-hidden">
        <div className="bg-linear-to-r from-gray-900 to-gray-700 dark:from-gray-800 dark:to-gray-600 px-6 py-8">
          <div className="flex items-center gap-6">
            {/* Profile Photo */}
            <div className="relative">
              <div className="w-24 h-24 bg-gray-200 dark:bg-gray-700 rounded-2xl flex items-center justify-center border-4 border-white dark:border-gray-600 shadow-lg">
              ...
              <button
                ...
                className="absolute -bottom-2 -right-2 w-8 h-8 bg-white dark:bg-gray-700 rounded-lg shadow-md flex items-center justify-center text-sm hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors"
              >
                📷
              </button>
            </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    disabled
                    className="w-full border border-border rounded-lg px-4 py-2.5 text-sm bg-surface-secondary text-text-tertiary"
                  />
                </div>
                <div>
                ...
                <div>
                  <label className="block text-sm font-medium text-text-secondary mb-1.5">No. Telepon</label>
                  <input
                    type="tel"
                ...
                <div>
                  <label className="block text-sm font-medium text-text-secondary mb-1.5">Instansi</label>
                  <input
                    type="text"
              ...
              <div className="flex justify-end">
                <button
                  ...
                  className="bg-accent text-surface px-6 py-2.5 rounded-lg hover:opacity-90 transition-all text-sm font-medium shadow-lg hover:shadow-xl"
                >
                  Simpan Perubahan
                </button>
              </div>
          ...
          {activeTab === "keamanan" && (
            <div className="max-w-lg space-y-6">
              <div className="bg-surface-secondary rounded-lg p-4 flex items-center justify-between">
                <div>
                  <h4 className="font-medium text-text-primary">Login Terakhir</h4>
                  <p className="text-sm text-text-tertiary">{accountInfo.loginTerakhir}</p>
                </div>
                <span className="text-green-600 dark:text-green-400">✓</span>
              </div>

              <div>
                <h3 className="font-semibold text-text-primary mb-4">Ubah Password</h3>
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-text-secondary mb-1.5">Password Saat Ini</label>
                    <input
                      type="password"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-text-secondary mb-1.5">Password Baru</label>
                    <input
                      type="password"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-text-secondary mb-1.5">Konfirmasi Password Baru</label>
                    <input
                      type="password"
                ...
                <button
                  ...
                  className="mt-4 bg-accent text-surface px-6 py-2.5 rounded-lg hover:opacity-90 transition-all text-sm font-medium"
                >
                  Ubah Password
                </button>
              </div>
                ...
                <table className="w-full">
                  <thead className="bg-surface-secondary">
                    <tr>
                      ...
                      <th className="text-left px-4 py-3 text-xs font-semibold text-text-secondary uppercase tracking-wider">Waktu</th>
                      <th className="text-left px-4 py-3 text-xs font-semibold text-text-secondary uppercase tracking-wider">IP Address</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border">
                    {recentActivities.map((activity) => (
                      <tr key={activity.id} className="hover:bg-surface-secondary transition-colors">
                        <td className="px-4 py-3 text-sm text-text-primary">{activity.aksi}</td>
                        <td className="px-4 py-3 text-sm text-text-secondary">{activity.waktu}</td>
                        <td className="px-4 py-3 text-sm text-text-tertiary font-mono">{activity.ip}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>

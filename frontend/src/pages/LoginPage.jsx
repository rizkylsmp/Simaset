...
const sampleAssets = [
  { id: 1, nama: "Tanah Alun-Alun", status: "aktif", lat: -7.6457, lng: 112.9061 },
  { id: 2, nama: "Gedung Kantor Pemkot", status: "aktif", lat: -7.6485, lng: 112.9095 },
  { id: 3, nama: "Tanah Pelabuhan", status: "berperkara", lat: -7.6350, lng: 112.9020 },
  { id: 4, nama: "Lapangan Untung Suropati", status: "aktif", lat: -7.6520, lng: 112.9150 },
  { id: 5, nama: "Taman Kota", status: "dijual", lat: -7.6400, lng: 112.9130 },
  { id: 6, nama: "Kawasan Industri", status: "aktif", lat: -7.6380, lng: 112.8950 },
  { id: 7, nama: "Terminal Bus", status: "tidak_aktif", lat: -7.6550, lng: 112.9200 },
];
...
export default function LoginPage() {
  ...
  const demoCredentials = [
    { label: "Admin", username: "admin", password: "admin123", color: "bg-gray-900" },
    { label: "Dinas Aset", username: "dinas_aset", password: "dinas123", color: "bg-emerald-600" },
    { label: "BPN", username: "bpn_user", password: "bpn123", color: "bg-blue-600" },
    { label: "Tata Ruang", username: "tata_ruang", password: "tataruang123", color: "bg-amber-600" },
  ];
    ...
    <div className="h-screen w-screen overflow-hidden relative bg-gray-900">
      ...
      <div className="absolute top-4 left-4 md:top-6 md:left-6 z-10 pointer-events-auto">
        <div className="flex items-center gap-2 md:gap-3 bg-gray-900/80 backdrop-blur-md rounded-xl md:rounded-2xl px-3 md:px-4 py-2 md:py-3 shadow-lg border border-white/10">
          <div className="w-8 h-8 md:w-10 md:h-10 bg-white rounded-lg md:rounded-xl flex items-center justify-center">
            <span className="text-base md:text-xl font-black text-gray-900">S</span>
          </div>
          <div>
            <h1 className="text-white font-bold text-sm md:text-lg tracking-tight">SINKRONA</h1>
            <p className="text-white/70 text-[10px] md:text-xs hidden sm:block">Sistem Manajemen Aset Tanah</p>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3 md:gap-4">
            {[
              ...
              <div key={item.label} className="flex items-center gap-1.5">
                ...
                <span className="text-white text-[10px] md:text-xs">{item.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div 
        className={`absolute top-0 right-0 h-full z-30 transition-all duration-500 ease-out ${
          showLoginPanel ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'
        }`}
      >
        <div className="h-full w-screen sm:w-[380px] md:w-[400px] bg-white flex flex-col shadow-2xl">
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="px-6 md:px-8 pt-10 md:pt-12 pb-6 md:pb-8 text-center bg-gradient-to-b from-gray-50 to-white">
              ...
              <h2 className="text-gray-900 font-bold text-lg md:text-xl">Masuk ke Akun</h2>
              <p className="text-gray-500 text-xs md:text-sm mt-1">Akses sistem manajemen aset tanah</p>
            </div>
            ...
            <div className="px-6 md:px-8 py-4 md:py-6">
              ...
              {error && (
                <div className="mb-4 md:mb-5 bg-red-50 border border-red-100 rounded-lg md:rounded-xl p-3 md:p-4 flex items-start gap-2 md:gap-3">
                  <div className="w-4 h-4 md:w-5 md:h-5 bg-red-100 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                    <svg className="w-2.5 h-2.5 md:w-3 md:h-3 text-red-600" fill="currentColor" viewBox="0 0 20 20">
              ...
              <form onSubmit={handleLogin} className="space-y-4 md:space-y-5">
                <div className="space-y-1.5 md:space-y-2">
                  <Label htmlFor="username" className="text-xs md:text-sm font-medium text-gray-700">
                    Username
                  </Label>
                  <Input
                    id="username"
                ...
                <Button
                  type="submit"
                  disabled={loading}
                  className="w-full h-10 md:h-12 bg-gray-900 hover:bg-gray-800 text-white text-sm font-semibold rounded-lg md:rounded-xl transition-all duration-200 shadow-lg hover:shadow-xl disabled:opacity-50"
                >
                  {loading ? (
            ...
            {/* Demo Credentials */}
            <div className="px-6 md:px-8 py-4 md:py-6 bg-gray-50 border-t border-gray-100">
                ...
                {demoCredentials.map((cred) => (
                  ...
                  >
                    <div className="flex items-center gap-1.5 md:gap-2 mb-0.5 md:mb-1">
                      <div className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full ${cred.color}`} />
                      <span className="font-semibold text-gray-900 text-[10px] md:text-xs">{cred.label}</span>
                    </div>
                    <p className="text-[9px] md:text-[10px] text-gray-400 font-mono truncate">{cred.username}</p>
                  </button>
                ))}
              </div>
            </div>

    ...
    return (
      <div className="text-center py-16 px-4">
        ...
        <p className="text-text-muted text-sm max-w-sm mx-auto">
          Data pengembalian akan muncul setelah proses pengembalian dilakukan
        </p>
      </div>
    );
  }
      ...
      <table className="w-full min-w-[900px]">
        <thead>
          <tr className="bg-linear-to-r from-surface-secondary to-surface border-b border-border">
            ...
            <ThSortable column="tanggal_pengembalian" className="min-w-[120px]">
              Tgl. Kembali
            </ThSortable>
            <ThSortable column="nilai_sewa" className="min-w-[120px]">
              Nilai Sewa
            </ThSortable>
            <th className="px-4 py-4 text-left text-[11px] font-semibold text-text-muted uppercase tracking-wider min-w-[120px]">
              Kondisi
            </th>
            <th className="px-4 py-4 text-center text-[11px] font-semibold text-text-muted uppercase tracking-wider w-20">
              Aksi
            </th>
          </tr>
        </thead>
        <tbody className="divide-y divide-border">
          {data.map((item, idx) => {
            const kondisiConfig = getKondisiConfig(item.kondisi_pengembalian);
            const KondisiIcon = kondisiConfig.icon;
            return (
              <tr
                key={item.id_sewa}
              ...
              >
                ...
                <td className="px-4 py-3.5">
                  <span
                    className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${kondisiConfig.bg} ${kondisiConfig.text} ${kondisiConfig.border}`}
                  >
                    <KondisiIcon size={13} weight="bold" />
                    {item.kondisi_pengembalian || "-"}
                  </span>
                </td>
                <td className="px-4 py-3.5">
